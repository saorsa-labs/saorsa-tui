...
impl<T> Tree<T> {
    ...
    fn render_border(&self, area: Rect, buf: &mut ScreenBuffer) {
        ...
        buf.set(x1, y1, Cell::new(tl, self.node_style.clone()));
        buf.set(x2, y1, Cell::new(tr, self.node_style.clone()));
        buf.set(x1, y2, Cell::new(bl, self.node_style.clone()));
        buf.set(x2, y2, Cell::new(br, self.node_style.clone()));

        for x in (x1 + 1)..x2 {
            buf.set(x, y1, Cell::new(h, self.node_style.clone()));
            buf.set(x, y2, Cell::new(h, self.node_style.clone()));
        }

        for y in (y1 + 1)..y2 {
            buf.set(x1, y, Cell::new(v, self.node_style.clone()));
            buf.set(x2, y, Cell::new(v, self.node_style.clone()));
        }
    }
}
