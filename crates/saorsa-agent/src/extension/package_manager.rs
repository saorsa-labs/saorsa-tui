...
impl PackageManager {
    ...
    pub fn uninstall(&mut self, name: &str) -> Result<()> {
        self.packages.remove(name).ok_or_else(|| {
    ...
    pub fn enable(&mut self, name: &str) -> Result<()> {
        let pkg = self.packages.get_mut(name).ok_or_else(|| {
            SaorsaAgentError::Extension(format!("extension '{}' not found", name))
        })?;
        pkg.enabled = true;
        self.save()?;
    ...
    pub fn load_package(&self, name: &str) -> Result<Box<dyn Extension>> {
        let _pkg = self.packages.get(name).ok_or_else(|| {
            SaorsaAgentError::Extension(format!("extension '{}' not found", name))
        })?;

        Err(SaorsaAgentError::Extension(
