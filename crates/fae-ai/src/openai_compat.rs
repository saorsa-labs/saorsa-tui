    ...
    fn headers(&self) -> Result<HeaderMap> {
        ...
        if !self.config.api_key.is_empty() {
            if let Some(custom_header) = &self.auth_header {
                let header_name = HeaderName::from_bytes(custom_header.as_bytes())
                    .map_err(|e| FaeAiError::Auth(format!("invalid auth header name: {e}")))?;
        ...
        for (name, value) in &self.extra_headers {
            let header_name = HeaderName::from_bytes(name.as_bytes())
                .map_err(|e| FaeAiError::Auth(format!("invalid header name '{name}': {e}")))?;
            let header_value = HeaderValue::from_str(value)
                .map_err(|e| FaeAiError::Auth(format!("invalid header value for '{name}': {e}")))?;
            headers.insert(header_name, header_value);
        }
...
/// Map an HTTP status code to the appropriate `FaeAiError`.
fn handle_http_error(status: reqwest::StatusCode, body: &str) -> FaeAiError {
    match status.as_u16() {
        401 | 403 => FaeAiError::Auth(format!(
            "OpenAI-Compatible auth error ({}): {}",
            status, body
        )),
        429 => FaeAiError::RateLimit(format!(
            "OpenAI-Compatible rate limit ({}): {}",
            status, body
        )),
        _ => FaeAiError::Provider {
...
impl Provider for OpenAiCompatProvider {
    async fn complete(&self, request: CompletionRequest) -> Result<CompletionResponse> {
        ...
        let body = serde_json::to_string(&compat_req).map_err(FaeAiError::Json)?;
        ...
        let compat_resp: CompatResponse =
            serde_json::from_str(&response_body).map_err(|e| FaeAiError::Provider {
                provider: "OpenAI-Compatible".to_string(),
