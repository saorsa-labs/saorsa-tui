...
fn handle_http_error(status: reqwest::StatusCode, body: &str) -> FaeAiError {
    match status.as_u16() {
        401 | 403 => FaeAiError::Auth(format!("Ollama auth error ({}): {}", status, body)),
        429 => FaeAiError::RateLimit(format!("Ollama rate limit ({}): {}", status, body)),
...
impl Provider for OllamaProvider {
    async fn complete(&self, request: CompletionRequest) -> Result<CompletionResponse> {
        ...
        let body = serde_json::to_string(&ollama_req).map_err(FaeAiError::Json)?;
