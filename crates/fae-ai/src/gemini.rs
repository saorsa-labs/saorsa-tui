...
fn handle_http_error(status: reqwest::StatusCode, body: &str) -> FaeAiError {
    match status.as_u16() {
        401 | 403 => FaeAiError::Auth(format!("Gemini auth error ({}): {}", status, body)),
        429 => FaeAiError::RateLimit(format!("Gemini rate limit ({}): {}", status, body)),
...
impl Provider for GeminiProvider {
    async fn complete(&self, request: CompletionRequest) -> Result<CompletionResponse> {
        ...
        let body = serde_json::to_string(&gemini_req).map_err(FaeAiError::Json)?;
