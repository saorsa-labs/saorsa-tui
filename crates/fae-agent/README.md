...
Agent runtime for the Fae AI framework, providing a complete environment for AI agents to execute tools and interact with the system.

## Overview

The `fae-agent` crate implements the core agent loop that enables AI models to:
...
```rust
use fae_agent::{Agent, ToolRegistry};
use fae_ai::AnthropicProvider;
...
```rust
use fae_agent::tools::{ReadTool, BashTool};
...
```rust
use fae_agent::{Tool, ToolRegistry};
