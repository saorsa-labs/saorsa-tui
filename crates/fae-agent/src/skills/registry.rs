...
impl SkillRegistry {
    ...
    ///
    /// Searches:
    /// - Project: .fae/skills/*.md
    /// - Global: ~/.fae/skills/*.md
    pub fn discover_skills() -> Vec<PathBuf> {
        let mut paths = Vec::new();

        // Project skills (.fae/skills/)
        let project_dir = PathBuf::from(".fae/skills");
        ...
        // Global skills (~/.fae/skills/)
        if let Some(home) = dirs::home_dir() {
            let global_dir = home.join(".fae/skills");
...
fn parse_skill_file(content: &str) -> Result<Skill> {
    ...
    if !trimmed.starts_with("---") {
        return Err(crate::error::FaeAgentError::Context(
    ...
    let end_pos = after_first.find("---").ok_or_else(|| {
        crate::error::FaeAgentError::Context("Front matter not properly closed".into())
    })?;
