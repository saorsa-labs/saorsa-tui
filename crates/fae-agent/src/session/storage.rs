...
impl SessionStorage {
    ...
    fn ensure_session_dir(&self, session_id: &SessionId) -> Result<(), FaeAgentError> {
    ...
    fn write_atomic(&self, path: &std::path::Path, data: &[u8]) -> Result<(), FaeAgentError> {
        let temp_path = path.with_extension("tmp");
        fs::write(&temp_path, data)
            .map_err(|e| FaeAgentError::Session(format!("Failed to write temp file: {}", e)))?;
        fs::rename(&temp_path, path)
            .map_err(|e| FaeAgentError::Session(format!("Failed to rename temp file: {}", e)))?;
        Ok(())
    }
    ...
    pub fn save_manifest(
        ...
        let json = serde_json::to_string_pretty(metadata)
            .map_err(|e| FaeAgentError::Session(format!("Failed to serialize manifest: {}", e)))?;
        self.write_atomic(&path, json.as_bytes())?;
        Ok(())
    }

    /// Load session metadata from manifest.json.
    pub fn load_manifest(&self, session_id: &SessionId) -> Result<SessionMetadata, FaeAgentError> {
        let path = self.session_dir(session_id).join("manifest.json");
        let json = fs::read_to_string(&path)
            .map_err(|e| FaeAgentError::Session(format!("Failed to read manifest: {}", e)))?;
        serde_json::from_str(&json)
            .map_err(|e| FaeAgentError::Session(format!("Failed to deserialize manifest: {}", e)))
    }
    ...
    pub fn save_tree(
        ...
        let json = serde_json::to_string_pretty(node)
            .map_err(|e| FaeAgentError::Session(format!("Failed to serialize tree: {}", e)))?;
        self.write_atomic(&path, json.as_bytes())?;
        Ok(())
    }

    /// Load session tree structure from tree.json.
    pub fn load_tree(&self, session_id: &SessionId) -> Result<SessionNode, FaeAgentError> {
        let path = self.session_dir(session_id).join("tree.json");
        let json = fs::read_to_string(&path)
            .map_err(|e| FaeAgentError::Session(format!("Failed to read tree: {}", e)))?;
        serde_json::from_str(&json)
            .map_err(|e| FaeAgentError::Session(format!("Failed to deserialize tree: {}", e)))
    }
    ...
    pub fn save_message(
        ...
        let json = serde_json::to_string_pretty(message)
            .map_err(|e| FaeAgentError::Session(format!("Failed to serialize message: {}", e)))?;
    ...
    pub fn load_messages(&self, session_id: &SessionId) -> Result<Vec<Message>, FaeAgentError> {
        ...
        let mut entries: Vec<_> = fs::read_dir(&messages_dir)
            .map_err(|e| {
                FaeAgentError::Session(format!("Failed to read messages directory: {}", e))
            })?
        ...
        for entry in entries {
            ...
            if path.extension().and_then(|s| s.to_str()) == Some("json") {
                let json = fs::read_to_string(&path).map_err(|e| {
                    FaeAgentError::Session(format!("Failed to read message file: {}", e))
                })?;
                let message: Message = serde_json::from_str(&json).map_err(|e| {
                    FaeAgentError::Session(format!("Failed to deserialize message: {}", e))
                })?;
...
impl Default for SessionStorage {
    fn default() -> Self {
        Self::new()
            .unwrap_or_else(|_| Self::with_base_path(PathBuf::from("/tmp/fae-sessions-fallback")))
    }
}
