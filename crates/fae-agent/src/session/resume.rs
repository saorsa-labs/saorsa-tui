...
) -> Result<Option<SessionId>, FaeAgentError> {
...
pub fn find_session_by_prefix(
    ...
    match matches.len() {
        0 => Err(FaeAgentError::Session(format!(
            "No session found matching prefix '{}'",
            prefix
        ))),
        1 => Ok(matches[0]),
        _ => Err(FaeAgentError::Session(format!(
...
) -> Result<(SessionMetadata, Vec<Message>), FaeAgentError> {
