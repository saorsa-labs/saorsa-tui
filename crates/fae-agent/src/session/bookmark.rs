...
impl BookmarkManager {
    ...
    fn load_bookmarks(&self) -> Result<HashMap<String, Bookmark>, FaeAgentError> {
    ...
    fn save_bookmarks(&self, bookmarks: &HashMap<String, Bookmark>) -> Result<(), FaeAgentError> {
    ...
    pub fn add_bookmark(&self, name: String, session_id: SessionId) -> Result<(), FaeAgentError> {
    ...
    /// Remove a bookmark.
    pub fn remove_bookmark(&self, name: &str) -> Result<bool, FaeAgentError> {
        let mut bookmarks = self.load_bookmarks()?;
    ...
    pub fn get_bookmark(&self, name: &str) -> Result<Option<Bookmark>, FaeAgentError> {
        let bookmarks = self.load_bookmarks()?;
        Ok(bookmarks.get(name).cloned())
    }

    /// List all bookmarks, sorted by name.
    pub fn list_bookmarks(&self) -> Result<Vec<Bookmark>, FaeAgentError> {
        let bookmarks = self.load_bookmarks()?;
    ...
    pub fn generate_auto_name(&self) -> Result<String, FaeAgentError> {
        let bookmarks = self.load_bookmarks()?;
        let mut counter = 1;
...
impl Default for BookmarkManager {
    fn default() -> Self {
        Self::new().unwrap_or_else(|_| Self::with_path(PathBuf::from("/tmp/fae-bookmarks.json")))
    }
}
