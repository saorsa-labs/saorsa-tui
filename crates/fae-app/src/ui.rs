...
fn render_header(state: &AppState, buf: &mut ScreenBuffer, area: Rect) {
    ...
    let header_text = format!(" fae | {} | {}", state.model, status_text);

    let style = Style::default()
        .fg(Color::Named(fae_core::color::NamedColor::Black))
        .bg(Color::Named(fae_core::color::NamedColor::White))
        .bold(true);
...
fn render_messages(state: &AppState, buf: &mut ScreenBuffer, area: Rect) {
    ...
    for (i, msg) in visible_messages.iter().enumerate() {
        ...
        let (prefix, style) = match &msg.role {
            ChatRole::User => (
                "> ",
                Style::default()
                    .fg(Color::Named(fae_core::color::NamedColor::Green))
                    .bold(true),
            ),
            ChatRole::Assistant => (
                "  ",
                Style::default().fg(Color::Named(fae_core::color::NamedColor::Cyan)),
            ),
            ChatRole::Tool { name } => {
                let prefix_str = format!("  [{name}] ");
                let style = Style::default()
                    .fg(Color::Named(fae_core::color::NamedColor::Yellow))
                    .dim(true);
            ...
            ChatRole::System => (
                "  ",
                Style::default()
                    .fg(Color::Named(fae_core::color::NamedColor::Magenta))
                    .italic(true),
            ),
        };
    ...
    if !state.streaming_text.is_empty() && !visible_messages.is_empty() {
        ...
        if y < area.position.y + area.size.height {
            let row_area = Rect::new(area.position.x, y, area.size.width, 1);
            let style = Style::default().fg(Color::Named(fae_core::color::NamedColor::Cyan));
