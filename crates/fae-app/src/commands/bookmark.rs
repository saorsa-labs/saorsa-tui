...
use fae_agent::{Bookmark, BookmarkManager, FaeAgentError, SessionId};
...
impl BookmarkCommand {
    /// Add a bookmark.
    pub fn add(session_id: SessionId, name: Option<String>) -> Result<String, FaeAgentError> {
    ...
    /// Remove a bookmark.
    pub fn remove(name: &str) -> Result<bool, FaeAgentError> {
        let manager = BookmarkManager::new()?;
        manager.remove_bookmark(name)
    }

    /// Rename a bookmark.
    pub fn rename(old_name: &str, new_name: String) -> Result<(), FaeAgentError> {
        let manager = BookmarkManager::new()?;
        manager.rename_bookmark(old_name, new_name)
    }

    /// List all bookmarks.
    pub fn list() -> Result<Vec<Bookmark>, FaeAgentError> {
        let manager = BookmarkManager::new()?;
        manager.list_bookmarks()
    }

    /// Jump to a bookmarked session.
    pub fn jump(name: &str) -> Result<Option<SessionId>, FaeAgentError> {
        let manager = BookmarkManager::new()?;
