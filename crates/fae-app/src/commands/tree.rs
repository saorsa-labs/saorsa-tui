...
use fae_agent::{
    FaeAgentError, SessionId, SessionStorage, TreeRenderOptions, build_session_tree, find_in_tree,
    render_tree,
};
use std::str::FromStr;
...
impl TreeCommand {
    ...
    pub fn execute(args: &str) -> Result<String, FaeAgentError> {
        ...
        if !args.trim().is_empty() {
            let session_id = SessionId::from_str(args.trim()).map_err(|_| {
                FaeAgentError::Session(format!("Invalid session ID: {}", args.trim()))
            })?;
